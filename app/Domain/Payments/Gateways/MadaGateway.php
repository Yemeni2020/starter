...
class MadaGateway implements PaymentGateway
{
    ...
    {
        $reference = 'MADA-' . Str::upper(Str::random(10));
        $redirectUrl = $this->buildRedirectUrl($order, $reference);

        return new PaymentInitResult('mada', $reference, $redirectUrl, [
    ...
    {
        $base = str_replace('{provider}', 'mada', config('payments.return_url'));
